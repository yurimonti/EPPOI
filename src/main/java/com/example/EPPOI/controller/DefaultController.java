...
public class DefaultController {
    ...
    @GetMapping("/prova/{username}")
    public ResponseEntity<?> provaUsername(@PathVariable String username){
        UserNode user = this.userNodeRepository.findByUsername(username);
        if(user instanceof EnteNode){
            EnteNode enteNode = (EnteNode)user;
            return ResponseEntity.ok(enteNode.getCity().getName());
        }
        return ResponseEntity.ok("Not instance of");
    }
    @GetMapping("/{poiId}")
    public ResponseEntity<?> poiId(@PathVariable String poiId) {
        Long id = Long.parseLong(poiId);
        CityNode result;
        try {
            result = this.cityService.getCityByPoi(id);
        }catch (Exception e) {
...
/*public void createPoiProva(CoordsRepository coordinateRepository, PoiRepository pointOfIntRepository,
    ...
    public ResponseEntity<List<PoiDTO>> getPois(){
        return ResponseEntity.ok(this.poiService.getAllPois().stream().map(PoiDTO::new)
                .toList());
    }
